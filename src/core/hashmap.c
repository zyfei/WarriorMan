...
static int swHashMap_node_delete(swHashMap_node *root,
		swHashMap_node *del_node);
...
static inline int swHashMap_node_add(swHashMap_node *root,
		swHashMap_node *add) {
	unsigned _ha_bkt;
	add->hh.next = NULL;
...
int swHashMap_add(swHashMap* hmap, const char *key, uint16_t key_len,
		void *data) {
...
static inline swHashMap_node *swHashMap_node_find(swHashMap_node *root,
		const char *key_str, uint16_t key_len) {
	swHashMap_node *out;
	...
	if (root) {
		...
		HASH_FIND_IN_BKT(root->hh.tbl, hh, (root)->hh.tbl->buckets[bucket],
				key_str, key_len, out);
	}
	return out;
}
...
static int swHashMap_node_delete(swHashMap_node *root,
	...
	if ((del_node->hh.prev == NULL) && (del_node->hh.next == NULL)) {
		...
		if (del_node == ELMT_FROM_HH(root->hh.tbl, root->hh.tbl->tail)) {
			root->hh.tbl->tail = (UT_hash_handle*) ((ptrdiff_t)(
					del_node->hh.prev) + root->hh.tbl->hho);
		}
		if (del_node->hh.prev) {
			((UT_hash_handle*) ((ptrdiff_t)(del_node->hh.prev)
					+ root->hh.tbl->hho))->next = del_node->hh.next;
		} else {
			DECLTYPE_ASSIGN(root, del_node->hh.next);
		}
		if (_hd_hh_del->next) {
			((UT_hash_handle*) ((ptrdiff_t) _hd_hh_del->next + root->hh.tbl->hho))->prev =
					_hd_hh_del->prev;
		}
...
int swHashMap_update(swHashMap* hmap, const char *key, uint16_t key_len,
		void *data) {
	swHashMap_node *root = hmap->root;
...
int swHashMap_move(swHashMap *hmap, const char *old_key, uint16_t old_key_len,
		const char *new_key, uint16_t new_key_len) {
	swHashMap_node *root = hmap->root;
...
static unsigned int crc32_tab[] = { 0x00000000, 0x77073096, 0xee0e612c,
