 ...
 * 创建一个协程socket
 */
wmCoroutionSocket * wmCoroutionSocket_init(int domain, int type, int protocol) {
	if (!wm_connections) {
...
wmString* wmCoroutionSocket_get_read_buffer(wmCoroutionSocket *socket) {
...
wmCoroutionSocket* wmCoroutionSocket_accept(wmCoroutionSocket *socket) {
	...
	do {
		...
		// errno == EAGAIN  队列慢了，继续尝试操作
			...
			&& wmCoroutionSocket_wait_event(socket, WM_EVENT_READ)); //说明此时没有客户端连接，我们就需要等待事件
	return wmCoroutionSocket_init_by_fd(connfd);
}

//读取
ssize_t wmCoroutionSocket_recv(wmCoroutionSocket *socket, int32_t length) {
	int ret;
	wmCoroutionSocket_get_read_buffer(socket);
	do {
		//第一次调用函数stSocket_accept来尝试获取客户端连接
		...
		// errno == EAGAIN  队列慢了，继续尝试操作
			...
			&& wmCoroutionSocket_wait_event(socket, WM_EVENT_READ)); //如果不能读，我们就需要等待事件
	return ret;
}
...
ssize_t wmCoroutionSocket_send(wmCoroutionSocket *socket, const void *buf,
	...
	do {
		...
		// errno == EAGAIN  队列慢了，继续尝试操作
			...
			&& wmCoroutionSocket_wait_event(socket, WM_EVENT_WRITE)); //说明此时没有客户端连接，我们就需要等待事件
	return ret;
}
