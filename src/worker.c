...
void wmWorker_checkEnv() {
	...
	// Process title.
	//setproctitle("%s: master process start_file=%s", _processTitle->str, _startFile->str);
	php_printf("%.*s: master process start_file=%.*s\n", (int) _processTitle->length, _processTitle->str, (int) _startFile->length, _startFile->str);
}
